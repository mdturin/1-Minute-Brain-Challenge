...
// ─── NUMBER SEQUENCE ─────────────────────────────────────────────────────────
function generateNumberSequence(difficulty: Difficulty): Puzzle {
  ...
  const sequence: number[] = Array.from({ length }, () => randomInt(1, 9));
  ...
  while (options.size < 4) {
    ...
    const useNearby = Math.random() < 0.6;
    const candidate = useNearby
      ? Math.max(1, Math.min(9, correctValue + randomInt(-2, 2)))
      : randomInt(1, 9);
...
function generateSumSequence(): Puzzle {
  const length = randomInt(5, 8);
  const sequence: number[] = Array.from({ length }, () => randomInt(1, 9));
  const idx1 = randomInt(0, length - 2);
  const idx2 = randomInt(idx1 + 1, length - 1);
...
// ─── COLOR SEQUENCE ───────────────────────────────────────────────────────────
function generateColorSequence(difficulty: Difficulty): Puzzle {
  const length = difficulty === 'easy' ? randomInt(3, 4) : difficulty === 'medium' ? randomInt(4, 6) : randomInt(5, 7);
  const sequence: string[] = Array.from({ length }, () => randomChoice(COLORS));
  const targetIndex = randomInt(0, length - 1);
  ...
  const arr = [...distractors, correctColor].sort(() => Math.random() - 0.5);
...
// ─── LETTER SEQUENCE ──────────────────────────────────────────────────────────
function generateLetterSequence(difficulty: Difficulty): Puzzle {
  const length = difficulty === 'medium' ? randomInt(4, 6) : randomInt(6, 8);
  const sequence: string[] = Array.from({ length }, () => randomChoice(LETTERS));
  const targetIndex = randomInt(0, length - 1);
  ...
  const distractors = wrongPool.sort(() => Math.random() - 0.5).slice(0, 3);
  const arr = [...distractors, correctLetter].sort(() => Math.random() - 0.5);
...
export function generateMemorySequencePuzzle(difficulty: Difficulty): Puzzle {
  if (difficulty === 'easy') {
    // Easy: numbers only
    return generateNumberSequence('easy');
  }

  if (difficulty === 'medium') {
    // Medium: numbers or colors
    const subtype = randomChoice(['numbers', 'colors'] as const);
    return subtype === 'colors'
      ? generateColorSequence('medium')
      : generateNumberSequence('medium');
  }

  // Hard: numbers, colors, letters, or sum
  const subtype = randomChoice(['numbers', 'colors', 'letters', 'sum'] as const);
  switch (subtype) {
    case 'colors':  return generateColorSequence('hard');
    case 'letters': return generateLetterSequence('hard');
    case 'sum':     return generateSumSequence();
    default:        return generateNumberSequence('hard');
  }
}
